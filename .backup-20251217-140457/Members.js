...
const Members = () => {
  ...
  const handleGenerateCredentials = async (member) => {
    try {
      const result = await generateMemberCredentials(member.id);
      dispatch(updateMember(result.member));
  ...
  const handleResetCredentials = async (member) => {
    if (window.confirm('Are you sure you want to reset this member\'s password?')) {
      try {
        const result = await resetMemberCredentials(member.id);
        dispatch(updateMember(result.member));
        setCredentials({ ...member.credentials, password: result.newPassword });
        setShowCredentials(true);
